...
public interface ResetSettings extends Cloneable {
  ...
  default <T extends ResetSettings> T as(Class<T> type) {
    return getClass().isAssignableFrom(type)
        ...
        : Objects.requireNonNull(
            null,
            "Tried to get reset settings as "
                + type.getSimpleName()
                + ", but it's not instance of "
                + type.getSimpleName()
                + ".");
  }
  ...
  default Timed asTimed() {
    return isTimed()
        ? (Timed) this
        : Objects.requireNonNull(
            null, "Tried to get reset settings as Timed, but it's not instance of Timed.");
  }
  ...
  default Percentage asPercentage() {
    return !isTimed()
        ...
        : Objects.requireNonNull(
            null,
            "Tried to get reset settings as Percentage, but it's not instance of Percentage.");
  }
