...
public class NmsHandler_1_8_R1 implements SuperiorNms {
  ...
  public void setBlock(
      @NonNull Chunk chunk, @NonNull Location location, @NonNull OMaterial material) {
    IBlockData data =
        dataMap.computeIfAbsent(material, mat -> Block.getByCombinedId(material.getCombinedId()));
    ...
    chunkSection.setType(
        location.getBlockX() & 15, location.getBlockY() & 15, location.getBlockZ() & 15, data);
  }
  ...
  public Map<OMaterial, OPair<Integer, List<ItemStack>>> getTypeAndDrops(Set<Location> locations) {
    ...
    for (Location location : locations) {
      if (world == null) world = ((CraftWorld) location.getWorld()).getHandle();

      net.minecraft.server.v1_8_R1.World finalWorld = world;
      net.minecraft.server.v1_8_R1.Chunk chunk =
          pairToChunk.computeIfAbsent(
              new OPair<>(location.getBlockX() >> 4, location.getBlockZ() >> 4),
              key -> finalWorld.getChunkAt(location.getBlockX() >> 4, location.getBlockZ() >> 4));
      ...
      IBlockData type =
          section.getType(
              location.getBlockX() & 15, location.getBlockY() & 15, location.getBlockZ() & 15);
  ...
  private List<org.bukkit.inventory.ItemStack> getDrops(
      Block block, net.minecraft.server.v1_8_R1.Chunk chunk) {
  ...
  public void refreshChunks(
      World world, Map<Chunk, Set<Location>> locations, Collection<Player> receivers) {
        ...
        (chunk, locs) -> {
          ...
          if (!usePacketChunk) {
            ...
            for (Location location : locs) {
              values[counter] =
                  (short)
                      ((location.getBlockX() & 15) << 12
                          | (location.getBlockZ() & 15) << 8
                          | location.getBlockY());
              counter++;
            }
