...
public class DropsHandler {
  ...
  static {
    ...
    register(
        OMaterial.LAPIS_ORE,
        () -> OMaterial.LAPIS_LAZULI.parseItem(ThreadLocalRandom.current().nextInt(4, 9)));
  ...
  public static ItemStack getDrop(OMaterial material) {
    return Optional.ofNullable(drops.get(material)).map(Supplier::get).orElse(material.parseItem());
  }
}
