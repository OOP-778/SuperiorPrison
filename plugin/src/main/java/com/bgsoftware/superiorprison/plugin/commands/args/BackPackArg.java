...
public class BackPackArg extends CommandArgument<BackPackConfig> {
  public BackPackArg() {
    ...
    setMapper(
        name -> {
          Optional<BackPackConfig> config =
              SuperiorPrisonPlugin.getInstance().getBackPackController().getConfig(name);
  ...
  public void onAdd(OCommand command) {
    command.nextTabComplete(
        (previous, args) ->
            SuperiorPrisonPlugin.getInstance().getBackPackController().getConfigs().keySet());
  }
}
