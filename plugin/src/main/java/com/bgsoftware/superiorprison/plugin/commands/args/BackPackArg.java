...
public class BackPackArg extends CommandArgument<AdvancedBackPackConfig> {
    public BackPackArg() {
        setDescription("a backpack identifier");
        setIdentity("backpack");
        setMapper(name -> {
            Optional<AdvancedBackPackConfig> config = SuperiorPrisonPlugin.getInstance().getBackPackController().getConfig(name);
