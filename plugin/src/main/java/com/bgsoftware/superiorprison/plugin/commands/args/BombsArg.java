...
public class BombsArg extends CommandArgument<BombConfig> {
    public BombsArg() {
        ...
        setMapper(in -> {
            Optional<BombConfig> bombOf = SuperiorPrisonPlugin.getInstance().getBombController().getBombOf(in);
    ...
    public void onAdd(OCommand command) {
        command.nextTabComplete((res, old) -> SuperiorPrisonPlugin.getInstance().getBombController().getBombs());
    }
}
