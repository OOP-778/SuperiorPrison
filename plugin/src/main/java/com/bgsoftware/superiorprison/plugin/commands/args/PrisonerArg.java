...
public class PrisonerArg extends CommandArgument<SPrisoner> {
  ...
  public PrisonerArg(boolean offline) {
    ...
    setMapper(
        prisoner -> {
          Optional<Prisoner> prisoner1 =
              SuperiorPrisonPlugin.getInstance().getPrisonerController().getPrisoner(prisoner);
          return new OPair<>(
              (SPrisoner) prisoner1.orElse(null), "Failed to find prisoner with name " + prisoner);
        });
  }
  ...
  public void onAdd(OCommand command) {
    command.nextTabComplete(
        (previous, args) ->
            SuperiorPrisonPlugin.getInstance().getPrisonerController().stream()
