...
public class CmdGive extends OCommand {
  public CmdGive() {
    ...
    argument(
        new IntArg().setIdentity("amount").setDescription("Amount of backpacks you want to give"));
    argument(
        new IntArg().setIdentity("level").setDescription("What level the backpacks should be"));
              ...
              .ensureSync(
                  () -> {
                    for (int i = 0; i < amount; i++) {
                      ...
                      else receiver.getInventory().addItem(backpack.getItem());
                    }
                  });
        });
  }
