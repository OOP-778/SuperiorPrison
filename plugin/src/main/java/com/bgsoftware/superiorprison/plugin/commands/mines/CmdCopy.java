          ...
          switch (type) {
            ...
            case GENERATOR:
              ...
              to.getGenerator()
                  ...
                  .addAll(from.getGenerator().getGeneratorMaterials());
            ...
            case EFFECTS:
              ...
              to.getEffects()
                  .addAll(
                      from.getEffects().get().stream()
                          .map(effect -> new SMineEffect(effect.getType(), effect.getAmplifier()))
                          .collect(Collectors.toSet()));
              break;

            case MESSAGES:
              from.getMessages().get().stream()
                  .map(message -> ((SMineMessage) message).clone())
                  .forEach(message -> to.getMessages().add(message));
              break;
