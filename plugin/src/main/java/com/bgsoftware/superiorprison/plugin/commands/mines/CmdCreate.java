...
public class CmdCreate extends OCommand {
  ...
  private final Cache<UUID, Boolean> creating =
  ...
  public CmdCreate() {
    ...
    label("create")
        ...
        .argument(new StringArg().setIdentity("name").setRequired(true))
        .listen(onCommand());
  }
  ...
  private Consumer<WrappedCommand> onCommand() {
    return command -> {
      ...
      addIfDoesntHave(
          SuperiorPrisonPlugin.getInstance().getMainConfig().getAreaSelectionTool().getItemStack(),
          player);
      ...
      AtomicReference<SubscribedEvent<PlayerInteractEvent>> posSelectEvent =
          new AtomicReference<>();
      posSelectEvent.set(
          sf.subscribeTo(
              PlayerInteractEvent.class,
              posEvent -> {
                if (!MutliVerUtil.isPrimaryHand(posEvent) || posEvent.getClickedBlock() == null)
                  return;
                ...
                if (regionPos1.get() == null) {
                  ...
                  if (!region
                      .get()
                      .containsLocation(posEvent.getClickedBlock().getLocation(), false)) {
                    LocaleEnum.MINE_CREATE_POSITION_MUST_BE_WITHIN_REGION
                        .getWithErrorPrefix()
                        .send(player);
                    return;
                  }
                  ...
                  messageBuilder(LocaleEnum.MINE_SELECT_SPAWN_POS.getWithPrefix()).send(command);
                  ...
                  sf.subscribeTo(
                      ...
                      spawnEvent -> {
                        ...
                        spawnPos.set(
                            new SPLocation(
                                spawnEvent.getPlayer().getEyeLocation().add(0.5, 0.8, 0.5)));
                      ...
                      new SubscriptionProperties<PlayerInteractEvent>()
                          ...
                          .filter(
                              spawnEvent ->
                                  spawnEvent.getPlayer().equals(player)
                                      && MutliVerUtil.isPrimaryHand(spawnEvent)
                                      && SuperiorPrisonPlugin.getInstance()
                                          .getMainConfig()
                                          .getAreaSelectionTool()
                                          .getItemStack()
                                          .equals(spawnEvent.getItem())));
                }
              },
              new SubscriptionProperties<PlayerInteractEvent>()
                  .timeOut(TimeUnit.MINUTES, 5)
                  ...
                  .filter(
                      posEvent ->
                          posEvent.getPlayer().equals(player)
                              && MutliVerUtil.isPrimaryHand(posEvent)
                              && SuperiorPrisonPlugin.getInstance()
                                  .getMainConfig()
                                  .getAreaSelectionTool()
                                  .getItemStack()
                                  .equals(posEvent.getItem())
                              && posEvent.getClickedBlock() != null)));
      ...
      sf.subscribeTo(
          ...
          new SubscriptionProperties<AsyncPlayerChatEvent>()
              ...
              .filter(
                  event ->
                      event.getMessage().equalsIgnoreCase("cancel")
