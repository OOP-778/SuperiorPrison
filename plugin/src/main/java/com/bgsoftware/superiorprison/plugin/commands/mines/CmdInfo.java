  ...
  public CmdInfo() {
    ...
    onCommand(
        command -> {
          ...
          message.replace(
              "{mine_icon}",
              new LineContent(Helper.beautify(OMaterial.matchMaterial(mine.getIcon())))
                  .hoverItem()
                  .item(mine.getIcon())
                  .parent());
          ...
          message.replace(
              "{mine_blocks_percentage}", mine.getGenerator().getBlockData().getPercentageLeft());
          message.replace(
              "{mine_materials}",
              mine.getGenerator().getGeneratorMaterials().stream()
