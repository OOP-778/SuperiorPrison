...
public enum LocaleEnum {
  ...
  EDIT_GENERATOR_WRITE_RATE(
      "Please write new rate! It must be higher than 0. Example: 5.0, 1.5, 0.1. Write 'cancel' to cancel the process"),
  ...
  EDIT_GENERATOR_SAVE_FAILED_WRONG_PERCENTAGE(
      "Failed to save, percentage must be equal to 100, no less or more!"),
  ...
  EDIT_MESSAGE_TITLE_CONTENT(
      new OChatMessage()
          ...
          .append(
              "   &7Fade in, stay, fade out: &d{message_fadeIn}, {message_stay}, {message_fadeOut}")
          .append("")
          .append("   &5Available functions")
  ...
  MINE_TELEPORT_FAILED_SPAWN_NOT_SET(
      "Failed to teleport to the mine, cause spawn point is not set!"),
  ...
  EDIT_ICON_MAIN_MESSAGE(
      new OChatMessage()
          ...
          .append(
              "   &d&l* &7set lore (line) (text) &5=>&f set specific line of the lore of the icon!")
          .append(
              "   &d&l* &7remove lore (line) &5=>&f removes specific line of the lore of the icon!")
          ...
          new ChatLine("Failed to rank up to (&d{rank_name}&7) cause requirements aren't met: ")
              .append(
                  new LineContent("{TEMPLATE}&d{requirement_type}")
                      .hover()
                      .add(
                          "&d&l* &7Expected Value: &d{requirement_expected}",
                          "&d&l* &7Current value: &d{requirement_current}")
                      .parent()))),
              ...
              new ChatLine("You have an rankup to (&d{rank_name}&7) available! ")
                  .append(
                      new LineContent("&d&lRANKUP")
                          ...
                          .command("rankup")
                          .parent()))),
  MAX_RANKUP_PRESTIGE_CHANGES(
      "You've ranked up from &d{starting_prestige} &7to &d{current_prestige}&7 prestige"),
  ...
  PRESTIGE_FAILED_DOES_NOT_MEET_REQUIREMENTS(
      new OChatMessage(
          new ChatLine(
                  "Failed to prestige up to (&d{prestige_name}&7) cause requirements aren't met: ")
              .append(
                  new LineContent("{TEMPLATE}&d{requirement_type}")
                      .hover()
                      .add(
  ...
  PRISONER_RANKS_VIEW(
      "&d{prisoner_name}&7 ranks information:",
      "&d&l* &7Ladder rank: &d{prisoner_ladderrank}",
      "&d&l* &7Special Ranks: &d{prisoner_specialranks}"),
  ...
  PRISONER_BOOSTER_ADD(
      "Added booster (&d{booster_type}, {booster_rate}, {booster_time}&7) to {prisoner_name}"),
  ...
  MINE_MESSAGES_CREATE_TYPE(
      "What type of message you want to create? Available: actionbar, title, chat"),
  ...
  SOLD_BLOCKS_MESSAGE(
      new OActionBarMessage()
          .text("&dYou've sold &5{blocks} &dblocks in the past &5{time}&d and earned &5{money}")),
  ...
  BACKPACK_DROPPED_INVENTORY_FULL(
      "Your backpack was dropped on the ground, because your inventory is full!"),
  BACKPACK_UPGRADE_DONT_MEET_REQUIREMENTS(
      "You cannot upgrade this backpack, because you don't meet the requirements!"),

  PRESTIGE_TOP_MESSAGE(
      new OChatMessage("&7&l&m------- &d&lPrestige Top", "{entries}")
          .append(new ChatLine("{TEMPLATE}&d&l{position} &d{player_name} &d{prestige}"))),
  ...
  MIGRATION_DUPLICATE(
      new OChatMessage(
              "&cThere has been duplicate object found in current data. Please select if you wanna to use the data from Current Database or From Source")
          .append("&c&l* &7Object Identity: &c{identity}")
          .append("")
          .append(
              new ChatLine()
                  .append(
                      new LineContent("&c&l[CURRENT]")
                          .hover()
                          .add("&cClick to select data from current database")
                          .parent()
                          .chat()
                          .message("current")
                          .parent())
                  .append(" ")
                  .append(
                      new LineContent("&c&l[SOURCE]")
                          .hover()
                          .add("&cClick to select data from source")
                          .parent()
                          .chat()
                          .message("source")
                          .parent()))),
  ...
  CANCELED_ACTION_CAUSE_MINE_RESET(
      "Cancelled your action. Cause mine is either resetting or initializing..."),
  ...
  @Getter private OMessage message;
    ...
    if (message instanceof OChatMessage) {
      if (cache[0] == null) {
        ...
        chatMessage
            .lineList()
            .get(0)
            .insert(new LineContent(PREFIX.message.asChat().lineList().get(0).raw()), 0);
        cache[0] = chatMessage;
        return chatMessage;
    ...
    if (message instanceof OChatMessage) {
      if (cache[1] == null) {
        ...
        chatMessage
            .lineList()
            .get(0)
            .insert(new LineContent(PREFIX_ERROR.message.asChat().lineList().get(0).raw()), 0);
        cache[1] = chatMessage;
        return chatMessage;
