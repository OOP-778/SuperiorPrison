  ...
  public boolean load() {
    ...
    try {
      ...
      JarUtil.copyFolderFromJar(
          "menus", dataFolder, JarUtil.CopyOption.COPY_IF_NOT_EXIST, SuperiorPrisonPlugin.class);

      // Update
      int updated = MenuUpdater.update("menus");
      if (updated != 0) getPlugin().getOLogger().print("Updated {} menu objects!", updated);

      for (File menuFile :
          Objects.requireNonNull(new File(dataFolder + "/menus").listFiles(File::isFile)))
