  ...
  public DatabaseController() {
    StorageInitializer.getInstance()
        .registerAdapter(
            ItemStack.class,
            true,
            new TypeAdapter<ItemStack>() {
              @Override
              public void write(JsonWriter writer, ItemStack itemStack) throws IOException {
                if (itemStack == null || itemStack.getType() == Material.AIR) writer.nullValue();
                else writer.value(serialize(itemStack).getAsString());
              }
    ...
    // SuperiorPrisonPlugin.getInstance().getMainConfig().getStorageSection();
    //
    //                    save(false, () -> {
    //                        for (Storage<?> storage : getStorages()) {
    //                            Storage currentImplementation = ((UniversalStorage)
    // storage).getCurrentImplementation();
    //                            Storage storageNew =
    // storageSection.getStorageProvider().apply((UniversalStorage) storage);
    //
    //                            if
    // (currentImplementation.getClass().equals(storageNew.getClass())) continue;
    //
