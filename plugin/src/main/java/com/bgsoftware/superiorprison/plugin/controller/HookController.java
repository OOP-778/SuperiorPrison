...
public class HookController {
    ...
    for (HookClassSupplier<?> clazzSupplier : hooks) {
      ...
      try {
        ...
        if (!hook.isLoaded()) {
          if (!hook.isRequired()) continue;
            ...
            throw new IllegalStateException(
                "Failed to hook into "
                    + hook.getPluginName()
                    + " because it's not loaded and it is required!");
        }
        ...
        SuperiorPrisonPlugin.getInstance()
            ...
            .print(
                "Hooked into ("
                    + hook.getPlugin().getName()
                    + ") "
                    + hook.getPlugin().getDescription().getVersion());
  ...
  public <T extends SHook> void executeIfFound(
      HookClassSupplier<T> supplier, Consumer<T> consumer) {
  ...
  public void disableIf(SHook sHook, boolean b, String s) {
    ...
    if (b) {
      SuperiorPrisonPlugin.getInstance()
          .getOLogger()
          .printWarning(sHook.getPluginName() + " failed to hook, cause: " + s);
    }
  }
