...
public class RankController
    implements com.bgsoftware.superiorprison.api.controller.RankController,
        OComponent<SuperiorPrisonPlugin>,
        LoadHookable<RankController> {
  ...
  @Getter private final List<Consumer<RankController>> loadHooks = new ArrayList<>();
  ...
  public boolean load() {
    ...
    try {
      Config ranksConfig =
          SuperiorPrisonPlugin.getInstance().getConfigController().getRanksConfig();
        ...
        if (section.isValuePresent("order")) {
          ...
          SLadderRank rank =
              new SLadderRank(
                  ...
                  section.isValuePresent("prefix")
                      ...
                      : defaultPrefix.replace("{rank_name}", section.getKey()),
          ...
          if (ladderRanks.containsKey(order)) {
            getPlugin()
                .getOLogger()
                .printWarning(
                    "Ladder rank with order " + order + " already exists, aborting rank creation!");
            continue;
          }
      ...
      throw new IllegalStateException(
          "Failed to load RankController cause " + thrw.getMessage(), thrw);
    }
  ...
  public Optional<Rank> getRank(String name) {
    return getRanks().stream().filter(rank -> rank.getName().contentEquals(name)).findFirst();
  }

  @Override
  public Optional<LadderRank> getLadderRank(String name) {
    return getLadderRanks().stream().filter(rank -> rank.getName().contentEquals(name)).findFirst();
  }
}
