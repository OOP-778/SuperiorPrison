...
public class RequirementController
    implements com.bgsoftware.superiorprison.api.controller.RequirementController {
  ...
  public void registerRequirement(Requirement req) {
    ...
    SuperiorPrisonPlugin.getInstance()
        .getOLogger()
        .print("Requirement (" + req.getId() + ") handler registered!");
  ...
  public void checkRequirements(Requirement req) {
    for (LoadingRequirementData data : loadingRequirementDataSet) {
      ...
      Constructor<? extends RequirementData> constr =
          req.getDataClazz().getDeclaredConstructor(Map.class);
  ...
  public OPair<String, RequirementData> parse(String toParse) {
    ...
    if (splitPart1.startsWith(" ")) splitPart1 = splitPart1.substring(1);
    ...
    if (type.isPresent()) {
      ...
      if (req.getDataClazz() == null) data.set(new DefaultParsedData(allData));
      else {
        try {
          Constructor<? extends RequirementData> constr =
              req.getDataClazz().getDeclaredConstructor(Map.class);
    ...
    } else data.set(new LoadingRequirementData(allData, null));
