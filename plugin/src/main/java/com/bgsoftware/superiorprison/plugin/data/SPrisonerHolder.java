...
public class SPrisonerHolder extends UniversalStorage<SPrisoner> implements PrisonerHolder {
  ...
  @Getter private final Map<String, UUID> usernameToUuidMap = Maps.newConcurrentMap();

  @Getter private final Map<UUID, SPrisoner> prisonerMap = new ConcurrentHashMap<>();
  ...
  public Optional<Prisoner> getPrisoner(String username) {
    ...
    if (uuid == null) {
      if (uuidPattern.matcher(username).matches()) return getPrisoner(UUID.fromString(username));
  ...
  public void cleanInvalids() {
    ...
    long count =
        stream()
            .filter(
                prisoner ->
                    prisoner.getOfflinePlayer() == null
                        || prisoner.getOfflinePlayer().getName() == null)
            .peek(this::remove)
            .count();
    ...
    SuperiorPrisonPlugin.getInstance()
        .getOLogger()
        .print(
            "Prisoner Invalidation DONE ({}) Took {}ms",
            count,
            (System.currentTimeMillis() - start));
  }
