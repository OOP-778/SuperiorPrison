...
public class BackPackListener {
  private final Pattern BACKPACK_UPGRADE_PATTERN =
      Pattern.compile("prison.backpack.autoupgrade.([^ ]+).([0-9]+)");
  ...
  public BackPackListener() {
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          if (!(event.getWhoClicked().getOpenInventory().getTopInventory().getHolder()
              instanceof BackpackLockable)) return;

          SPrisoner viewer =
              ((BackpackLockable)
                      event.getWhoClicked().getOpenInventory().getTopInventory().getHolder())
                  .getViewer();
          ...
          viewer
              ...
              .ifPresent(
                  pair -> {
                    if (event.getSlot() == pair.getFirst()) event.setCancelled(true);
                  });
        });
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          SBackPack backPack =
              (SBackPack)
                  SuperiorPrisonPlugin.getInstance()
                      ...
                      .getBackPack(itemStack, event.getPlayer());
          SPrisoner prisoner =
              SuperiorPrisonPlugin.getInstance()
          ...
          List<String> permissions =
              PermUtil.getPermissions(BACKPACK_UPGRADE_PATTERN, event.getPlayer());
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          SPlayerInventory patchedInventory =
              ((PatchedInventory) event.getClickedInventory()).getOwner();
          ...
          new OTask().delay(100).runnable(patchedInventory::init).execute();
        });
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          SPlayerInventory inventory =
              ((PatchedInventory) event.getPlayer().getInventory()).getOwner();
          ...
          SuperiorPrisonPlugin.getInstance()
              .getOLogger()
              .printDebug("Player {} Dropped Backpack", event.getPlayer().getName());
        });
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          ((PatchedInventory) event.getPlayer().getInventory())
              .getOwner()
              .getBackPackMap()
              .forEach(
                  (key, backpack) -> {
