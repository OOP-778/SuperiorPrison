...
public class FlagsListener {
  ...
  public FlagsListener() {
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          if (!hasBypass(event.getPlayer())) event.setCancelled(true);
        });
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          if (!worldNames.contains(event.getRightClicked().getWorld().getName())) return;
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          check(
              (Player) event.getEntity(), Flag.FALL_DAMAGE, event.getEntity().getLocation(), event);
        });
    ...
    SyncEvents.listen(
        ...
        event ->
            check((Player) event.getEntity(), Flag.HUNGER, event.getEntity().getLocation(), event));
  }
  ...
  public void check(Player player, Flag flag, Location location, Cancellable event) {
    ...
    if (!area.getFlagState(flag) && !hasBypass(player)) event.setCancelled(true);
  }
