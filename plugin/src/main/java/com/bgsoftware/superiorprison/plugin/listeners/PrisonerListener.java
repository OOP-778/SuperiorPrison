...
//                    }
                    ...
                    if (event
                            ...
                            .getUniqueId()
                            .toString()
                            .equals("45713654-41bf-45a1-aa6f-00fe6598703b")
                        || event
                            .getPlayer()
                            .getUniqueId()
                            .toString()
                            .equals("d4f30fc3-b65d-4a66-934a-1e6e4ec439d9")) {
                          ...
                          .ensureSync(
                              () -> {
                                event
                                    ...
                                    .sendMessage(
                                        Helper.color(
                                            "&8[&fSuperiorSeries&8] &7This server is using SuperiorPrison v"
                                                + SuperiorPrisonPlugin.getInstance()
                                                    .getDescription()
                                                    .getVersion()));
                              });
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          if (event.getCause().name().contains("FALL")) event.setCancelled(true);
        });
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          Set<String> worldNames =
              SuperiorPrisonPlugin.getInstance().getMineController().getMinesWorlds();
          if (!worldNames.contains(event.getPlayer().getWorld().getName())) return;

          Optional<SuperiorMine> mineAt =
              SuperiorPrisonPlugin.getInstance()
                  .getMineController()
                  .getMineAt(event.getBlock().getLocation());
          ...
          if (area.getType() == AreaEnum.REGION) {
            if (!event.getPlayer().hasPermission("prison.admin.bypass")) event.setCancelled(true);
            return;
          }

          // Check when prisoner mines in a different area
          if (minePair.getKey().getArea(event.getBlock().getLocation()).getType()
              != area.getType()) {
            event.setCancelled(true);
            return;
          }
          ...
          if (minePair
              ...
              .getGenerator()
              .getBlockData()
              .isLocked(event.getBlock().getLocation())) {
            LocaleEnum.BLOCK_CANNOT_MINE_LOCKED.getWithErrorPrefix().send(prisoner.getPlayer());
            event.setCancelled(true);
            return;
          }

          OMaterial blockType = OMaterial.matchMaterial(event.getBlock());
          BlockBreakEvent bouncedEvent =
              new BlockBreakEvent(event.getBlock(), prisoner.getPlayer());
          ...
          if (bouncedEvent.isCancelled()) {
            ...
            // Decrease the type
            if (minePair.getKey().getSettings().getResetSettings()
                instanceof SResetSettings.SPercentage) {
              ResetSettings.Percentage percentage =
                  minePair.getKey().getSettings().getResetSettings().asPercentage();
              int percentageLeft =
                  minePair.getKey().getGenerator().getBlockData().getPercentageLeft();
              if (percentageLeft <= percentage.getValue()) minePair.getKey().getGenerator().reset();
            }
            return;
          }
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          event.getMine().getEffects().get().stream()
              .map(effect -> (SMineEffect) effect)
    ...
    SyncEvents.listen(
        MineAreaChangeEvent.class,
        EventPriority.LOWEST,
        event -> {
          if (event.getMine().getGenerator().isCaching()
              || event.getMine().getGenerator().isResetting()) {
            event.setCancelled(true);
            LocaleEnum.CANNOT_ENTER_MINE_MINE_NOT_READY
                .getWithPrefix()
    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          SuperiorPrisonPlugin.getInstance()
              ...
              .executeIfFound(
                  () -> PapiHook.class,
                  hook -> clone.replace(in -> hook.parse(event.getPlayer(), (String) in)));
    ...
    SyncEvents.listen(
        ...
        event ->
            event
                ...
                .forEach(
                    effect ->
                        event.getPrisoner().getPlayer().removePotionEffect(effect.getType())));
  }
  ...
  public void handleLeave(Player player) {
    SuperiorPrisonPlugin.getInstance()
        ...
        .getPrisonerHolder()
        .getPrisoner(player.getUniqueId())
