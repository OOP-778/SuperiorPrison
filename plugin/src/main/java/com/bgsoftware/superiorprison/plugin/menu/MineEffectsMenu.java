        ...
        .handle(
            event -> {
              ...
              if (event.getClick().name().contains("LEFT")) {
                ...
                } else mineEffect.setAmplifier(mineEffect.getAmplifier() + 1);
  ...
  public OMenuButton toButton(PotionEffectType obj) {
    try {
      ...
      if (itemStack.getType().name().toLowerCase().contains("potion")) {
        ...
        if (OVersion.isOrAfter(12)) itemMeta.setColor(obj.getColor());
      ...
      return effectButton.currentItem(
          defaultStateItem.getItemStackWithPlaceholders(
              ImmutableMap.of(
                  "{potion_amplifier}",
                  amplifier,
                  "{potion_type}",
                  Helper.beautify(obj.getName()))));
    } catch (Throwable th) {
      th.printStackTrace();
    }
    return null;
