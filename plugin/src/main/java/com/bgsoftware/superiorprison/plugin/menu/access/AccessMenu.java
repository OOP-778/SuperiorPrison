...
public abstract class AccessMenu extends OPagedMenu<AccessObject> implements OMenu.Templateable {
  ...
  public AccessMenu(String identifier, SPrisoner viewer) {
    ...
    ClickHandler.of("sort click")
        .handle(
            event -> {
              SortMethod next = sortMethod;
              if (event.getClick().isLeftClick()) next = sortMethod.getNext();
              else if (event.getClick().isRightClick()) next = sortMethod.getPrevious();
              ...
              sortMethod = next;
              if (sortMethod != SortMethod.INPUT) refresh();
            })
        .apply(this);
  }
  ...
  public List<AccessObject> sorted(Stream<AccessObject> stream) {
    if (sortMethod == SortMethod.ABC)
      ...
      stream =
          stream.sorted(
              (access1, access2) -> {
                if (access1.isInstanceOf(SLadderRank.class)
                    && access2.isInstanceOf(SLadderRank.class)) return 0;
                else if (access1.isInstanceOf(SLadderRank.class)) return -1;
      ...
      stream =
          stream.sorted(
              (access1, access2) -> {
                if (access1.isInstanceOf(SSpecialRank.class)
                    && access2.isInstanceOf(SSpecialRank.class)) return 0;
                else if (access1.isInstanceOf(SSpecialRank.class)) return -1;
      ...
      if (input == null) {
        ...
        stream =
            stream.sorted(
                (access1, access2) -> {
                  if (access1.getName().toLowerCase().startsWith(input)
                      && access2.getName().toLowerCase().startsWith(input)) return 0;
                  else if (access1.getName().startsWith(input)) return -1;
          ...
          stream.sorted(
              (access1, access2) -> {
                if (access1.isInstanceOf(Prestige.class) && access2.isInstanceOf(Prestige.class))
                  return 0;
                else if (access1.isInstanceOf(Prestige.class)) return -1;
