...
public class FindAccessMenu extends AccessMenu {
    ...
    ClickHandler.of("access click")
        .handle(
            event -> {
              ...
              } else mine.addPrestige(accessObject.getAs(Prestige.class));
  ...
  public List<AccessObject> requestObjects() {
    ...
    SuperiorPrisonPlugin.getInstance().getRankController().getRanks().stream()
        .filter(rank -> !mine.getRanks().contains(rank.getName()))
        .map(AccessObject::new)
        .forEach(accessObjects::add);

    SuperiorPrisonPlugin.getInstance().getPrestigeController().getPrestiges().stream()
        .filter(prestige -> !mine.getPrestiges().contains(prestige.getName()))
        .map(AccessObject::new)
        .forEach(accessObjects::add);

    return sorted(accessObjects.stream().sorted(Comparator.comparing(AccessObject::getName)));
  }
  ...
  public OMenuButton toButton(AccessObject obj) {
    ...
    return button.currentItem(
        defaultStateItem.getItemStackWithPlaceholdersMulti(getViewer(), mine, obj));
  }
