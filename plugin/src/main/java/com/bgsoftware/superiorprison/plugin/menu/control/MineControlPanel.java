...
public class MineControlPanel extends OPagedMenu<OptionEnum> implements OMenu.Templateable {
  ...
  public Inventory getInventory() {
    ...
    Optional<OMenuButton.ButtonItemBuilder> soon =
        getTemplateButtonFromTemplate("option").map(button -> button.getStateItem("soon"));
  ...
  private void handleIconClick(ButtonClickEvent event) {
    ...
    chatCommands.appendCommand(
        ...
        (player, canceller, args) -> {
          ...
          mine.save(true);
          refreshMenus(
              getClass(), menu -> menu.getMine().getName().contentEquals(getMine().getName()));
          refresh();
          canceller.set(true);
    ...
    chatCommands.appendCommand(
        ...
        (player, args) -> {
          ...
          Preconditions.checkArgument(
              material != null, "Failed to find material by name: " + args[0]);
    ...
    chatCommands.appendCommand(
        ...
        (player, args) -> {
          ...
          Preconditions.checkArgument(
              itemBuilder[0].getOMaterial() == OMaterial.PLAYER_HEAD,
              "This command only works for heads!");

          itemBuilder[0] = new OSkull(itemBuilder[0].getItemStack()).texture(args[0]);
          messageBuilder(LocaleEnum.EDIT_ICON_SET_TEXTURE.getWithPrefix()).send(player);
        });
    ...
    chatCommands.afterInput(
        () -> {
          ...
          clone.replace(
              "%item%",
              new LineContent(
                      Helper.beautify(OMaterial.matchMaterial(itemBuilder[0].getItemStack())))
                  .hoverItem()
                  .item(itemBuilder[0].getItemStack())
                  .parent());
    ...
    clone.replace(
        "%item%",
        new LineContent(Helper.beautify(OMaterial.matchMaterial(itemBuilder[0].getItemStack())))
            .hoverItem()
            .item(itemBuilder[0].getItemStack())
            .parent());
    clone.send(event.getWhoClicked());
  ...
  public OMenuButton toButton(OptionEnum obj) {
    ...
    Preconditions.checkArgument(
        templateButtonFromTemplate.isPresent(), "Failed to find option template in Control Panel");
    ...
    if (mine.getLinker().isLinked(obj.name().toLowerCase())) {
      ...
      if (stateItem != null) {
        stateItem
            .itemBuilder()
            .replace(
                "{linked_mine_name}", mine.getLinker().getLinkedTo().get(obj.name().toLowerCase()))
            .replace("{option_name}", obj.name().toLowerCase());
        return optionButton.currentItem(stateItem.getItemStack());
      }
    }

    OMenuButton.ButtonItemBuilder optionItem =
        optionButton.getStateItem(obj.name().toLowerCase() + "-option");
