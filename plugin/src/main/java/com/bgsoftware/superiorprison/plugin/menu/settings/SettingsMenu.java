...
public class SettingsMenu extends OPagedMenu<SettingsObject> implements OMenu.Templateable {
                  ...
                  .onError(
                      (input, err) -> {
                        messageBuilder(LocaleEnum.EDIT_SETTINGS_ERROR.getWithErrorPrefix())
                            ...
                            .send(input.player());
                        if (err.getMessage() == null) getEngine().getLogger().error(err);
                      })
                  .listen();
            });
  }
  ...
  public OMenuButton toButton(SettingsObject obj) {
    ...
    if (state == null) state = settingButton.getDefaultStateItem();

    state.itemBuilder().replace("{setting_value}", obj.currentValue().toString().toLowerCase());
