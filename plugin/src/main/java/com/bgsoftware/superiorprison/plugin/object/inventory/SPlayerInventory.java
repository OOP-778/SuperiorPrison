...
public class SPlayerInventory {
  ...
  static {
    ...
    try {
      Class<?> patchedInventoryClass =
          Class.forName("com.bgsoftware.superiorprison.plugin.nms.PlayerInventory_" + version);
      Class<?> craftPlayerInventoryClass =
          OSimpleReflection.findClass("{cb}.inventory.CraftInventoryPlayer");
      ...
      Method craftGetNmsInventory =
          OSimpleReflection.getMethod(craftPlayerInventoryClass, "getInventory");
      Constructor<?> patchedInventoryConstructor =
          OSimpleReflection.getConstructor(
              patchedInventoryClass, nmsPlayerInventoryClass, SPlayerInventory.class);
      ...
      patcher =
          (player) -> {
            ...
            try {
              Object patchedInventory =
                  patchedInventoryConstructor.newInstance(
                      craftGetNmsInventory.invoke(inventoryField.get(player)),
                      new SPlayerInventory(player));
              inventoryField.set(player, patchedInventory);

              SuperiorPrisonPlugin.getInstance()
                  .getOLogger()
                  .printDebug("Patched {} inventory!", player.getName());
      ...
      throw new IllegalStateException(
          "Unsupported version " + version + ". Failed to find PlayerInventory, contact author!",
          e);
    }
  }
  ...
  @Getter private Map<Integer, SBackPack> backPackMap = new ConcurrentHashMap<>();
  ...
  public ItemStack[] addItem(ItemStack... itemStacks) {
    ...
    for (SBackPack backpack : backPackMap.values()) {
      ...
      SuperiorPrisonPlugin.getInstance()
          .getOLogger()
          .printDebug(
              "Added items to a backpack of {} players. Left: {}", player.getName(), add.size());

      // Added all the items
      if (add.isEmpty()) return new ItemStack[0];
      else itemStacks = add.keySet().toArray(new ItemStack[0]);
    }
  ...
  public void init() {
    ...
    SuperiorPrisonPlugin.getInstance()
        .getOLogger()
        .printDebug("Found {} backpacks in {} inventory", backPackMap.size(), player.getName());
  }
  ...
  public ItemStack setItem(int slot, ItemStack itemStack) {
    if (itemStack == null || itemStack.getType() == Material.AIR) {
      ...
      if (sBackPack != null) backPackMap.remove(slot);
      return itemStack;
    }
    ...
    if (getBPC().isBackPack(itemStack)) {
      ...
      if (uuid != null && currentBackpack != null && uuid.equals(currentBackpack.getUuid()))
        return itemStack;
      else {
        SBackPack pack =
            (SBackPack)
  ...
  public SBackPack findBackPackBy(ItemStack itemStack) {
    ...
    return backPackMap.values().stream()
  ...
  public int getSlotByBackPack(SBackPack backPack) {
    return getBackPackMap().entrySet().stream()
