...
public class SMineBlockData implements Attachable<SMineGenerator>, MineBlockData {
  ...
  @Getter
  private final Map<OTriplePair<Integer, Integer, Integer>, OMaterial> locToMaterial =
      new ConcurrentHashMap<>();

  @Getter private final Map<OMaterial, OPair<Long, Long>> materials = new ConcurrentHashMap<>();

  @Getter
  private final OCache<SBLocksLock, Boolean> lockedBlocks =
      OCache.builder().concurrencyLevel(1).expireAfter(5, TimeUnit.SECONDS).build();

  private SMineGenerator generator;

  @Getter @Setter private long blocksLeft = 0;
  ...
  public void set(Location location, OMaterial material) {
    ...
    OPair<Long, Long> data = materials.computeIfAbsent(material, in -> new OPair<>(0L, 0L));
  ...
  public void remove(Location location) {
    ...
    materials.merge(
        material,
        new OPair<>(1L, 0L),
        (f, s) -> f.setFirst(Math.max(f.getFirst() - s.getFirst(), 0L)));
  ...
  public void unlock(Lock lock) {
    ...
    for (Location lockedLocation : ((SBLocksLock) lock).lockedLocations) remove(lockedLocation);
  }
  ...
  public Optional<Lock> getLockAt(Location location) {
    return lockedBlocks.keySet().stream()
        .filter(lock -> lock.getLockedLocations().contains(location))
        .map(lock -> (Lock) lock)
        .findFirst();
  }
