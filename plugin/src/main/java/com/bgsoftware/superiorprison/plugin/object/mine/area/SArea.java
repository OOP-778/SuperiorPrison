...
public class SArea implements Area, Attachable<SNormalMine>, SerializableObject {
  ...
  @Getter private final Map<Flag, Boolean> flags = Maps.newConcurrentMap();
  @Getter private transient SNormalMine mine;
  ...
  public boolean isInsideWithY(SPLocation location, boolean yDownwards) {
    ...
    return location.xBlock() >= x1
        ...
        && location.zBlock() >= z1
        && location.zBlock() <= z2
        && (!yDownwards ? location.y() >= y1 && location.y() <= y2 : location.y() >= y1);
  }
  ...
  public void attach(SNormalMine obj) {
    ...
    for (Flag flag : Flag.values())
      if (!flags.containsKey(flag)) flags.put(flag, flag.getDefaultValue());
  }
            ...
            pair -> {
              ...
              try {
                ...
                SuperiorPrisonPlugin.getInstance()
                    .getOLogger()
                    .printWarning(
                        "Failed to find a flag by {}, ignoring it...", key.applyAs(String.class));
              }
            });
  }
}
