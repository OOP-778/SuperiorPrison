...
public abstract class SMineMessage implements MineMessage, SerializableObject, Cloneable {
  ...
  @Setter @Getter private transient ZonedDateTime timeToRun;
  ...
  public static SMineMessage from(SerializedData data) {
    ...
    if (type.equalsIgnoreCase("chat")) message = new SMineChatMessage();
    else if (type.equalsIgnoreCase("title")) message = new SMineTitleMessage();
    else if (type.equalsIgnoreCase("actionbar")) message = new SMineActionBarMessage();
