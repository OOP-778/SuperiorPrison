...
public class SMineTitleMessage extends SMineMessage implements MineTitleMessage {
  ...
  @Getter private Optional<String> subTitle;
  ...
  public SMineTitleMessage(
      long every, int fadeIn, int fadeOut, int stay, String title, String subTitle) {
    super(every);
    this.fadeIn = fadeIn;
  ...
  public void send(CommandSender sender) {
    ...
    OTitle.sendTitle(
        ...
        stay,
        fadeOut,
        title[0].trim().length() == 0 ? null : title[0],
        subTitle[0].trim().length() == 0 ? null : subTitle[0],
        ((Player) sender));
  }
