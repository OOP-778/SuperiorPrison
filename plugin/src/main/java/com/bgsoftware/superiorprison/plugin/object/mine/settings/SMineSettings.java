...
@Setter
@AllArgsConstructor
public class SMineSettings
    implements Attachable<SNormalMine>,
        com.bgsoftware.superiorprison.api.data.mine.settings.MineSettings,
        SerializableObject,
        LinkableObject<SMineSettings> {
  ...
  public void deserialize(SerializedData data) {
    ...
    this.disableEnderPearls =
        data.getChildren("disableEnderPearls").map(sd -> sd.applyAs(boolean.class)).orElse(false);

    this.disableMonsterSpawn =
        data.getChildren("disableMonsterSpawn").map(sd -> sd.applyAs(boolean.class)).orElse(false);
  ...
  public void setResetType(ResetType resetType) {
    ...
    if (resetType == ResetType.PERCENTAGE) settings = new SResetSettings.SPercentage(60);
    else settings = new SResetSettings.STimed(TimeUnit.MINUTES.toSeconds(10));
  ...
  public void onChange(SMineSettings from) {
    ...
    if (resetSettings instanceof Attachable) ((Attachable) this.resetSettings).attach(mine);
  }
