...
@Getter
public class SShop
    implements MineShop, Attachable<SNormalMine>, SerializableObject, LinkableObject<SShop> {
  ...
  public static SShop from(SShop from) {
    ...
    from.items.stream().map(SShopItem::from).forEach(item -> shop.items.add(item));
  ...
  public boolean hasItem(ItemStack itemStack) {
    return getItems().stream().anyMatch(shopItem -> shopItem.getItem().isSimilar(itemStack));
  }
