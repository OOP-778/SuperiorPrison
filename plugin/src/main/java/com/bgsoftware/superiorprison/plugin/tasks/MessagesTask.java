    ...
    runnable(
        () ->
            SuperiorPrisonPlugin.getInstance()
                ...
                .forEach(
                    mine -> {
                      mine.getMessages().get().stream()
                          .map(message -> (SMineMessage) message)
                          .forEach(
                              message -> {
                                if (message.getTimeToRun() == null) {
                                  message.setTimeToRun(
                                      getDate().plusSeconds(message.getInterval()));
                                  return;
                                }

                                Duration between =
                                    Duration.between(getDate(), message.getTimeToRun());
                                if (between.getSeconds() <= 0) {
                                  mine.getPrisoners()
                                      .forEach(prisoner -> message.send(prisoner.getPlayer()));
                                  message.setTimeToRun(
                                      getDate().plusSeconds(message.getInterval()));
                                }
                              });
