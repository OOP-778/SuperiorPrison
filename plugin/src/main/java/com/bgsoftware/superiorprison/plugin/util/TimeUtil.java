...
public class TimeUtil {
  public static long toSeconds(String string) {
    ...
    List<String> characters =
        Arrays.stream(string.split("\\d")).filter(s -> !s.isEmpty()).collect(Collectors.toList());
  ...
  public static String leftToString(ZonedDateTime date, boolean reverse) {
    ...
    Duration between;
    if (reverse) between = Duration.between(date, getDate());
    else between = Duration.between(getDate(), date);
