    ...
    SyncEvents.listen(
        ...
        event -> {
          ...
          if (!(event.getWhoClicked().getOpenInventory().getTopInventory().getHolder()
              instanceof OMenu)) return;

          OMenu menu =
              (OMenu) event.getWhoClicked().getOpenInventory().getTopInventory().getHolder();
          if (!(event.getClickedInventory().getHolder() instanceof OMenu)) {
            menu.handleBottomClick(event);
            return;
          }

          if (event.getClickedInventory().getHolder() == menu) menu.handleClick(event);
        });
