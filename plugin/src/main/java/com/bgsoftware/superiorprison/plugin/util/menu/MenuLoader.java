...
public class MenuLoader {
    ...
    for (int a = 0; a < layout.size(); a++) {
      ...
      for (int b = 0; b < row.length(); b++) {
        ...
        if (menu instanceof OMenu.Templateable
          ...
          ((OMenu.Templateable) menu)
              .getTemplateButtonMap()
              .put(
                  ((OMenu.Templateable) menu).getTemplateFromIdentifier(ch + "").orElse("-"),
                  button);
          slot++;
          continue;
        }
  ...
  private static OMenuButton initButton(ConfigSection section) {
    ...
    if (section.isValuePresent("material"))
      button.addState(
          "default", new OMenuButton.ButtonItemBuilder(ItemBuilder.fromConfiguration(section)));

    for (ConfigSection stateSection : section.getSections().values()) {
      button.addState(
          stateSection.getKey(),
          new OMenuButton.ButtonItemBuilder(ItemBuilder.fromConfiguration(stateSection)));
    }
