...
public class OMenuButton implements Cloneable {
  ...
  @Setter private ItemStack currentItem;
  @Setter private String requiredPermission = "";
  ...
  public ButtonItemBuilder getStateItem(String state) {
    ...
    if (aDefault == null)
      return new ButtonItemBuilder(
          new OItem(OMaterial.RED_STAINED_GLASS_PANE)
              .setDisplayName("&cFailed to find a button state by id '" + state + "'"));
  ...
  public static class ButtonItemBuilder {
    ...
    public <T> ItemStack getItemStackWithPlaceholders(T object) {
      ...
      Optional<PapiHook> papiOptional =
          SuperiorPrisonPlugin.getInstance().getHookController().findHook(() -> PapiHook.class);
      if (!papiOptional.isPresent()) return itemBuilder.getItemStack();
      PapiHook papi = papiOptional.get();

      Set<OPair<String, Function<Object, String>>> placeholdersFor =
    ...
    public <T> ItemStack getItemStackWithPlaceholders(
        T object, Set<OPair<String, Function<T, String>>> placeholders) {
      if (getItemStack().getType() == Material.AIR) return getItemStack();
      ItemBuilder clone = itemBuilder.clone();
    ...
    private List<String> finalizeLore(List<String> lore, Player player) {
      ...
      for (String line : lore) {
        if (line.startsWith("<hp=")) {
          ...
          ClassDebugger.debug(
              player.getName() + " Has Permission " + extractPermission(line) + ": " + hasPerm);
          continue;
        ...
        if (inTags && hasPerm) finalizedLore.add(line);
        else if (!inTags) finalizedLore.add(line);
      }
    ...
    private Player getPlayer(Object object) {
      ...
      if (object instanceof SPrisoner && ((SPrisoner) object).isOnline())
        return ((SPrisoner) object).getPlayer();
      return null;
    }
  }
}
