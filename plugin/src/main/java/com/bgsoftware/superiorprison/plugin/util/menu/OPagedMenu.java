  ...
  public Inventory getInventory() {
        ...
        Objects.requireNonNull(
            buildInventory(
                getTitle()
                    ...
                    .replace("{pages_available}", pages + ""),
                getViewer()),
            "Invalid Inventory");

    if (allItems.isEmpty()) {
      buttonOf(button -> button.action().contentEquals("next page"))
          .ifPresent(
              button -> {
                ...
                inventory.setItem(
                    ...
                    button
                        .currentItem(
                            button
                                .getStateItem("hidden")
                                .getItemStackWithPlaceholdersMulti(getViewer()))
                        .currentItem());
              });

      buttonOf(button -> button.action().contentEquals("previous page"))
          .ifPresent(
              button -> {
                if (!button.containsState("hidden")) return;
    ...
    for (int i = 0; i < emptySlots.size(); i++) {
      ...
      if (pagedButton.isPresent()) {
        ...
        inventory.setItem(
            emptySlots.get(i),
            new OItem(OMaterial.RED_STAINED_GLASS_PANE)
                .setDisplayName("&cFailed to convert object into paged button")
                .getItemStack());
    }

    Optional<OMenuButton> nextPageButton =
        buttonOf(button -> button.action().contentEquals("next page"));
    Optional<OMenuButton> previousPageButton =
        buttonOf(button -> button.action().contentEquals("previous page"));
      ...
      if (currentPage == pages) {
        if (button.containsState("hidden"))
          inventory.setItem(
              ...
              button
                  .currentItem(
                      button.getStateItem("hidden").getItemStackWithPlaceholdersMulti(getViewer()))
                  .currentItem());
        else inventory.setItem(button.slot(), OMaterial.AIR.parseItem());
      } else
        inventory.setItem(
            button.slot(),
            button
                .currentItem(button.getStateItem("shown").getItemStackWithPlaceholders(getViewer()))
                .currentItem());
    }
    ...
    if (previousPageButton.isPresent()) {
      ...
      if (currentPage == 1)
        if (button.containsState("hidden"))
          ...
          inventory.setItem(
              previousPageButton.get().slot(),
              button.currentItem(OMaterial.AIR.parseItem()).currentItem());
      else
        inventory.setItem(
            button.slot(),
            button
  ...
  public List<Integer> getEmptySlots() {
    ...
    if (this instanceof Placeholderable)
      getFillerItems()
          .forEach(
              (slot, button) -> {
                if (((Placeholderable) this).containsPlaceholder(button.identifier() + "")) return;

                usedSlots.add(slot);
              });
    else usedSlots.addAll(getFillerItems().keySet());

    usedSlots.addAll(buttons.keySet());
    for (int slot = 0; slot < (getMenuRows() * 9); slot++) {
      if (!usedSlots.contains(slot)) emptySlots.add(slot);
    }
